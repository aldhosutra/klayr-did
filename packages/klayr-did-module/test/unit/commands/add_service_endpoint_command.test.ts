...
import { AddServiceEndpointCommand } from '@dist/commands/add_service_endpoints_command';
...
import { AddServiceEndpointEvent } from '@dist/events/add_service_endpoint_event';
import {
  chainID,
  config,
  customDID,
...
type CommandParam = AddServiceEndpointParam;
const MODULE_NAME = 'did';
const COMMAND_NAME = 'addServiceEndpoint';
const commandSchema = utils.schema.addServiceEndpointCommandSchema;
...
describe('AddServiceEndpointCommand', () => {
  let module: DidModule;
  let store: StoreGetter;
  let command: AddServiceEndpointCommand;
  let documentSubstore: DocumentStore;
  let nonceSubstore: NonceStore;

  beforeEach(async () => {
    module = new DidModule();
    command = new AddServiceEndpointCommand(module.stores, module.events);
  ...
  describe('constructor', () => {
    it('should inherit from BaseCommand', () => {
      expect(AddServiceEndpointCommand.prototype).toBeInstanceOf(BaseCommand);
    });
  ...
  describe('execute', () => {
    it('should keep service endpoint empty for specified did document if this command is not executed', async () => {
      const didDocument = await documentSubstore.get(store, documentStoreKey(senderDID));
      expect(didDocument.service).toHaveLength(0);
    });

    it('should add service endpoint for specified did document', async () => {
    ...
    it('should add command events', async () => {
      const context = createCommandExecuteContext(validParam);
      await command.execute(context);
      expect(eventResult(context.eventQueue, AddServiceEndpointEvent, MODULE_NAME, validParam)).toBe(true);
    });
  });
});
