...
import { KLAYR_DID_PREFIX } from '../../utils/constant';
...
export abstract class BaseDriver {
  ...
  async computeId(keyPair: Ed25519VerificationKey2020) {
    ...
    const Klayr32 = cryptography.address.getKlayr32AddressFromPublicKey(
      await decodePublicKey(keyPair.publicKeyMultibase),
    );
    return `${KLAYR_DID_PREFIX}:${this.chainspace}:address:${Klayr32}#${keyPair.fingerprint()}`;
  }
  ...
  async _keyPairToDidDocument(keyPair: Ed25519VerificationKey2020, chainspace: string) {
    ...
    const did = `${KLAYR_DID_PREFIX}:${chainspace}:address:${Klayr32}`;
