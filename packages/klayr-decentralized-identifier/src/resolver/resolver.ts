...
import { KLAYR_DID_METHOD_NAME } from '../utils/constant';

export interface CachedResolver {
  method?: typeof KLAYR_DID_METHOD_NAME;
  use?: (driver: any) => void;
...
class CachedResolverSingleton {
  ...
  public static getIPCInstance(ipc: string) {
    if (this.ipcResolver === undefined || this.prevIpc !== ipc) {
      const didKlayrDriver = didResolver.driver.createOffChainDriver({ ipc });
      this.ipcResolver = new CachedResolverBuilder();
      this.ipcResolver.use!(didKlayrDriver);
      this.ipcResolver.method = KLAYR_DID_METHOD_NAME;
    }
  ...
  public static getWSInstance(ws: string) {
    if (this.wsResolver === undefined || this.prevWs !== ws) {
      const didKlayrDriver = didResolver.driver.createOffChainDriver({ ws });
      this.wsResolver = new CachedResolverBuilder();
      this.wsResolver.use!(didKlayrDriver);
      this.wsResolver.method = KLAYR_DID_METHOD_NAME;
    }
...
export function createResolver(options: CreateResolverParam): CachedResolver {
  ...
  const ret: CachedResolver | object = { method: KLAYR_DID_METHOD_NAME };
